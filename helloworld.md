...
# คู่มือการซิงค์ข้อมูลให้เป็นหนึ่งเดียว เมื่อทำงานหลายเครื่อง (Git Syncing)

สมมติสถานการณ์: คุณมี **เครื่อง A (คอมพิวเตอร์ที่บ้าน)** และ **เครื่อง B (โน้ตบุ๊กพกพา)** และทั้งสองเครื่องติดตั้ง Git ไว้เรียบร้อยแล้ว

---

## สเต็ปที่ 1: การนำโปรเจกต์ไปลงในเครื่องใหม่ (ทำครั้งเดียวที่เครื่อง B)

เมื่อคุณเปิดโน้ตบุ๊ก (เครื่อง B) เป็นครั้งแรกและอยากทำงานต่อ คุณต้อง "โคลนนิ่ง" (Clone) โปรเจกต์จาก GitHub ลงมาที่เครื่องนี้ก่อนครับ

1. เปิด Command Prompt บนเครื่อง B
2. พิมพ์คำสั่ง `cd` เพื่อไปยังโฟลเดอร์ที่คุณต้องการเก็บงาน (เช่น `cd D:\pusit\projects`)
3. พิมพ์คำสั่ง Clone (ก๊อปปี้ URL มาจากหน้าเว็บ GitHub ของคุณ):
   ```bash
   git clone [https://github.com/pusit-ai/comorg-book-thai.git](https://github.com/pusit-ai/comorg-book-thai.git)
   ```
4. เข้าไปในโฟลเดอร์ที่เพิ่งโหลดมา:
   ```bash
   cd comorg-book-thai
   ```
*(ตอนนี้เครื่อง B มีไฟล์ทุกอย่างเหมือนเครื่อง A เป๊ะ และพร้อมทำงานแล้วครับ)*

---

## สเต็ปที่ 2: ก่อนลุกจากเครื่อง A (ส่งงานขึ้นคลาวด์)

สมมติคุณพิมพ์งานที่บ้าน (เครื่อง A) เสร็จแล้ว กำลังจะปิดคอมไปข้างนอก **ห้ามปิดเครื่องเฉยๆ เด็ดขาด!** คุณต้องเซฟและส่งงานขึ้น GitHub ก่อนครับ:

1. ดูสถานะไฟล์:
   ```bash
   git status
   ```
2. เอาไฟล์ทั้งหมดเข้าคิวเซฟ:
   ```bash
   git add .
   ```
3. บันทึกประวัติ:
   ```bash
   git commit -m "อัปเดตเนื้อหาบทที่ 4 ทำที่คอมบ้าน"
   ```
4. ส่งขึ้น GitHub:
   ```bash
   git push origin main
   ```
*(ตอนนี้งานล่าสุดของคุณไปรออยู่บนอินเทอร์เน็ตแล้ว)*

---

## สเต็ปที่ 3: เมื่อมาเปิดเครื่อง B ทำงานต่อ (ดึงงานลงมาอัปเดต)

พอมาถึงมหาลัย เปิดโน้ตบุ๊ก (เครื่อง B) ขึ้นมา **ห้ามเริ่มพิมพ์งานทันที!** เพราะไฟล์ในเครื่อง B ยังเป็นเวอร์ชันเก่าอยู่ คุณต้องดึงงานล่าสุดจากเว็บลงมาก่อน:

1. เปิด Command Prompt เข้าไปที่โฟลเดอร์โปรเจกต์
2. **ดึงงานล่าสุดลงมาอัปเดตเครื่อง (สำคัญมาก⭐):**
   ```bash
   git pull origin main
   ```
   *(ระบบจะดาวน์โหลดไฟล์ที่คุณแก้จากเครื่อง A มาทับในเครื่อง B ให้อัตโนมัติ)*
3. เริ่มทำงาน พิมพ์เนื้อหา หรือแก้ไขไฟล์ตามปกติ
4. พอทำเสร็จ ก็ทำซ้ำ สเต็ปที่ 2 คือ `add`, `commit`, และ `push` ขึ้นไปใหม่

---

## สเต็ปที่ 4: วนลูปกลับมาที่เครื่อง A

พอกลับมาถึงบ้าน เปิดคอมเครื่อง A **อย่าลืมดึงงานก่อนเริ่มทำ!**
1. พิมพ์คำสั่งดึงงาน:
   ```bash
   git pull origin main
   ```
2. ทำงานต่อ -> `add` -> `commit` -> `push`

วนลูปแบบนี้ไปเรื่อยๆ ข้อมูลของคุณจะตรงกันเป๊ะในทุกเครื่อง และไม่มีวันสูญหายแน่นอนครับ!
...
* **ใช้ GitHub Desktop:**
    * ปุ่ม `Fetch origin` (ด้านบน) = ดึงอัปเดต (`pull`)
    * ปุ่ม `Commit` (ด้านล่างซ้าย) = เซฟประวัติ (`add` + `commit`)
    * ปุ่ม `Push origin` (ด้านบน) = ส่งขึ้นเว็บ (`push`)